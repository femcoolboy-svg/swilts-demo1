...
io.on('connection', (socket) => {
    ...
    socket.on('private-message', (data) => {
        ...
        if (target) io.to(target).emit('private-message', { from: data.from, msg: data.msg, time: new Date() });
    });
    ...
    socket.on('group-message', (data) => {
        ...
        db.all(`SELECT user_id FROM group_members WHERE group_id = ?`, [data.group], (err, members) => {
            members.forEach(m => {
                ...
                if (target && m.user_id !== data.from) io.to(target).emit('group-message', { group: data.group, from: data.fromName, msg: data.msg, time: new Date() });
            });
        });
    });
...
app.post('/register', (req, res) => {
    ...
    if (!username || !email || !password || !captcha) return res.json({ success: false, error: 'Заполните поля' });
        ...
        db.get(`SELECT tag FROM users ORDER BY CAST(tag AS INTEGER) DESC LIMIT 1`, (err, row) => {
            ...
            bcrypt.hash(password, 10, (err, hash) => {
                ...
                db.run(`INSERT INTO users (username, email, password_hash, tag, ip) VALUES (?, ?, ?, ?, ?)`,
                    [username, email, hash, nextTag, ip], function(err) {
                        if (err) return res.json({ success: false, error: 'Ошибка' });
                        req.session.user = { id: this.lastID, username, tag: nextTag, role: 'user', created_at: new Date() };
    ...
    db.get(`SELECT * FROM users WHERE username = ?`, [username], (err, user) => {
        ...
        bcrypt.compare(password, user.password_hash, (err, result) => {
            ...
            db.get(`SELECT plan, expires_at FROM subscriptions WHERE user_id = ?`, [user.id], (err, sub) => {
                ...
                req.session.user = {
                    id: user.id, username: user.username, tag: user.tag, role: user.role,
                    avatar: user.avatar, banner: user.banner, created_at: user.created_at,
                    plus_color: user.plus_color, plus_badge: user.plus_badge,
...
app.post('/plus/settings', (req, res) => {
    ...
    if (req.session.user) { req.session.user.plus_color = color; req.session.user.plus_badge = badge; }
    res.json({ success: true });
});
...
app.get('/all-users', (req, res) => {
    ...
    db.all(`SELECT id, username, tag, banned, role, created_at FROM users`, (err, users) => res.json({ users }));
});
...
app.post('/unban', (req, res) => {
    if (req.session.user?.username !== 'prisanok') return res.json({ success: false });
    db.run(`UPDATE users SET banned = 0, ban_reason = '' WHERE id = ?`, [req.body.userId]);
    res.json({ success: true });
});
...
app.post('/search', (req, res) => {
    const { q } = req.body;
    if (q === 'prisanok0') return res.json({ success: true, isDiscord: true, id: '1175045445928632382' });
    db.all(`SELECT id, username, tag FROM users WHERE (username LIKE ? OR tag LIKE ?) AND banned = 0 LIMIT 10`, [`${q}%`, `${q}%`], (err, rows) => {
...
app.post('/friend/add', (req, res) => {
    ...
    db.get(`SELECT * FROM friends WHERE (user1_id=? AND user2_id=?) OR (user1_id=? AND user2_id=?)`, [from, to, to, from], (err, f) => {
        if (f) return res.json({ success: false, error: 'Уже друзья' });
        db.get(`SELECT * FROM friend_requests WHERE from_user_id=? AND to_user_id=?`, [from, to], (err, r) => {
            if (r) return res.json({ success: false, error: 'Запрос уже отправлен' });
...
app.post('/friends', (req, res) => {
    db.all(`SELECT u.id, u.username, u.tag, u.avatar, u.plus_badge, u.created_at FROM friends f JOIN users u ON f.user2_id = u.id WHERE f.user1_id = ? AND u.banned = 0`, [req.body.userId], (err, rows) => {
        res.json({ friends: rows || [] });
    });
});

app.post('/requests', (req, res) => {
    db.all(`SELECT fr.id, u.id as user_id, u.username, u.tag FROM friend_requests fr JOIN users u ON fr.from_user_id = u.id WHERE fr.to_user_id = ?`, [req.body.userId], (err, rows) => {
...
app.post('/group/create', (req, res) => {
    ...
    let max = 15;
    db.get(`SELECT plan FROM subscriptions WHERE user_id = ? AND expires_at > datetime('now')`, [owner], (err, sub) => { if (sub && sub.plan !== 'free') max = 50;
        const all = [...new Set([owner, ...members])];
        if (all.length < 3 || all.length > max) return res.json({ success: false, error: `От 3 до ${max} человек` });
...
app.post('/groups', (req, res) => {
    db.all(`SELECT g.id, g.name FROM group_chats g JOIN group_members gm ON g.id = gm.group_id WHERE gm.user_id = ?`, [req.body.userId], (err, rows) => {
...
app.post('/group/invite', (req, res) => {
    ...
    db.get(`SELECT allow_group_invite FROM users WHERE id = ?`, [to], (err, u) => {
        if (u && u.allow_group_invite === 0) return res.json({ success: false, error: 'Пользователь запретил приглашения' });
...
app.post('/group/invites', (req, res) => {
    db.all(`SELECT gi.id, gi.group_id, gc.name as group_name, u.username as fromName FROM group_invites gi JOIN group_chats gc ON gi.group_id = gc.id JOIN users u ON gi.from_user_id = u.id WHERE gi.to_user_id = ?`, [req.body.userId], (err, rows) => {
        res.json({ invites: rows || [] });
    });
});

app.post('/group/messages', (req, res) => {
    db.all(`SELECT gm.*, u.username as fromName FROM group_messages gm JOIN users u ON gm.from_user_id = u.id WHERE gm.group_id = ? ORDER BY timestamp ASC`, [req.body.groupId], (err, rows) => {
        res.json({ messages: rows || [] });
    });
});
...
app.post('/avatar-gif', upload.single('file'), (req, res) => {
    ...
    db.get(`SELECT plan, expires_at FROM subscriptions WHERE user_id = ?`, [req.session.user.id], (err, sub) => {
        const hasPlus = sub && sub.plan !== 'free' && new Date(sub.expires_at) > new Date();
        if (!hasPlus) { fs.unlink(req.file.path, ()=>{}); return res.json({ success: false, error: '❌ Только для SWILTS+' }); }
        if (path.extname(req.file.originalname).toLowerCase() !== '.gif') { fs.unlink(req.file.path, ()=>{}); return res.json({ success: false, error: 'Только GIF' }); }
        const url = `/uploads/${req.file.filename}`;
...
app.post('/banner-video', upload.single('file'), (req, res) => {
    ...
    db.get(`SELECT plan, expires_at FROM subscriptions WHERE user_id = ?`, [req.session.user.id], (err, sub) => {
        ...
        if (ext !== '.mp4' && ext !== '.webm') { fs.unlink(req.file.path, ()=>{}); return res.json({ success: false, error: 'Только MP4/WEBM' }); }
        const url = `/uploads/${req.file.filename}`;
